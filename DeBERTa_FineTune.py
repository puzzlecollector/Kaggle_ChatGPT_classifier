...
hc3_df = pd.DataFrame(list(zip(prompts, answers, labels)), columns=["prompt", "answer", "AI"])

input_ids, attn_masks = [], [] 

prompts = hc3_df["prompt"].values 
answers = hc3_df["answer"].values 
labels = hc3_df["AI"].values
...
for idx, (train_idx, val_idx) in enumerate(kf.split(input_ids, labels)): 
    ...
    for epoch_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        if best_val_loss > avg_val_loss: 
            ...
            torch.save(model.state_dict(), f"DeBERTaLarge_HC3_{idx+1}.pt")
